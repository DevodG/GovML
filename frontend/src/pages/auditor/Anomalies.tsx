...
export default function AuditorAnomalies() {
  ...
  const loadAnomalies = async () => {
    try {
      ...
      const data = await auditorAPI.getAnomalies(params)
  ...
  return (
    <div className="animate-fade-in">
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 28 }}>
        ...
        <h1 style={{ fontSize: 20, fontWeight: 700, letterSpacing: '-0.02em' }}>Anomalies</h1>
      </div>
      ...
      <DataTable
        ...
        actions={(anomaly) => [
          {
            label: 'View Details',
            onClick: () => (window.location.href = `/auditor/anomalies/${anomaly.id}`),
            icon: <Eye size={14} />,
          },
        ]}
